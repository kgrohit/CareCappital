...
/*The section to display pages by limits*/
	...
	if($rows != 'all') {
		...
		if( $_POST['desc'] ) {
			$data = $connection->query("SELECT * FROM DisciplineMaster WHERE Description = '$desc'");
		} else {
			$data = $connection->query("SELECT * FROM DisciplineMaster") or die(mysql_error()); 
		}
		$data->execute();
...
if($desc == "" ) {
	$sql = "SELECT * FROM DisciplineMaster ORDER BY DisciplineID ASC $max" ;
}
...
if($desc != "" ) {
	...
	if($desc == '*'){
		$sql = "SELECT * FROM DisciplineMaster WHERE Description = '$desc'  ORDER BY DisciplineID ASC $max" ;
...
?>
	...
	foreach($connection->query($sql) as $row) {  //inserting the statement here
		...
		print "id='hover' onclick='editForm(" . $row['DisciplineID'] .")' >";
		print "<td width = 10%>" . $row['DisciplineID'] . "</td>";
